SELECT
  ...
  COUNT(DISTINCT popularity.user_id)                              AS popular_score,
  MAX(sl.created_at)                                              AS restocked_at
FROM stock_logs sl
JOIN items i ON i.item_id = sl.item_id
JOIN item_products ip ON ip.item_id = i.item_id
JOIN wishlists target_wishlist ON target_wishlist.item_id = i.item_id AND target_wishlist.user_id = ?
JOIN wishlists popularity ON popularity.item_id = i.item_id
...
ORDER BY popular_score DESC, restocked_at DESC
LIMIT 1
